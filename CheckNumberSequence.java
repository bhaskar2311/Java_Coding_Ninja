...
public class CheckNumberSequence {
	...
	public static void main(String[] args) {
        ...
        // Edge case: If there is only one or no elements
        if (n <= 1) {
            System.out.println("true");
            return;
        }

        int prev = sc.nextInt(); // First element
        boolean isDecreasing = true; // Initially assume the sequence is decreasing

        for (int i = 1; i < n; i++) {
            int current = sc.nextInt();

            if (isDecreasing) {
                // If the sequence should be decreasing but found an increase
                if (current > prev) {
                    isDecreasing = false; // Change to increasing mode
                }
            } else {
                // If the sequence should be increasing but found a decrease
                if (current <= prev) {
            ...
            prev = current; // Update previous element for next iteration
        }
